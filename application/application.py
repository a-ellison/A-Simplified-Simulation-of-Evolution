...
class Application(tkinter.Tk):
    def __init__(self, window_width=DEFAULT_WINDOW_WIDTH, window_height=DEFAULT_WINDOW_HEIGHT):
        ...
        self.simulation_controller = SimulationController(self.canvas, self.canvas_width, self.canvas_height,
                                                          self.behavior, self.speed, self.seed)
    ...
    def create_radio_param(self, param, count):
        ...
        for option in options:
            radio = tkinter.Radiobutton(radio_frame, text=option, variable=radio_var, value=option,
    ...
    def create_entry_param(self, param, count):
        ...
        entry = tkinter.Spinbox(self.params_frame, from_=0, textvariable=var, validatecommand=validate,
                                **DEFAULT_SPINBOX)
        label.grid(column=count, row=0)
