...
class SimulationController:
    def __init__(
        self, canvas, canvas_width, canvas_height, behavior_var, speed_var, seed_var
    ):
        self.canvas = canvas
    ...
    def update_canvas(self):
        ...
        for drawable in self.simulation.all_drawables:
            if drawable.canvas_id is None:
                drawable.canvas_id = self.canvas.create_circle(
                    drawable.position.x,
                    drawable.position.y,
                    drawable.radius,
    ...
    def setup(self):
        ...
        self.simulation = Simulation(
            self.canvas_width, self.canvas_height, behavior, self.seed, self.config
        )
        self.update_canvas()
    ...
    def show_message(self, msg):
        ...
        self.canvas.create_text(
            self.canvas_width / 2, self.canvas_height / 2, fill="white", text=msg
        )
