...
import models.primer.primer_behavior as behavior


# TODO: display deaths and births
class PrimerData(object):
    ...
    def plot_population(self):
        fig, ax = plt.subplots()
        ax.plot([i for i in range(1, self.days + 1)], self.population)
    ...
    def plot_traits(self):
        fig, ax = plt.subplots()
        relative_radius_average = self.relative_average(self.average_radius, PrimerAnimal.MIN_RADIUS, PrimerAnimal.MAX_RADIUS)
        relative_speed_average = self.relative_average(self.average_speed, PrimerAnimal.MIN_SPEED, PrimerAnimal.MAX_SPEED)
        relative_sight_range_average = self.relative_average(self.average_sight_range, PrimerAnimal.MIN_SIGHT_RANGE, PrimerAnimal.MAX_SIGHT_RANGE)
        days = [i for i in range(1, self.days + 1)]
